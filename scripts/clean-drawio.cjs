...
for (const pattern of removeNodePatterns) {
  ...
  while ((m = re2.exec(xml)) !== null) {
    ...
    const idMatch = xml
      .substring(cellStart, m.index + m[0].length + 200)
      .match(/id="([^"]+)"/);
...
function cleanExternalResource(val) {
  ...
  if (/cloudfront[\s]+\d+/.test(val))
    return "CloudFront Distribution (external)";
...
function cleanLabel(val) {
  ...
  if (/^(prod|root)-\w+Stack$/.test(v)) {
    return v.replace(/^(prod|root)-/, "").replace(/Stack$/, "");
  }

  // Strip prodapp or rootapp prefix
  v = v.replace(/^(prodapp|rootapp)/, "");
  ...
  if (/rule.*allow-event-rule/i.test(lower)) {
    return (
      splitConcatenated(lower.split("-allow-event-rule")[0]) + " (permission)"
    );
  }
  ...
  if (/lambda-errors-/.test(lower)) {
    const pathMatch = lower.match(
      /lambda-errors-(get|post|delete|head|put|patch)(.*)/,
    );
    if (pathMatch) {
